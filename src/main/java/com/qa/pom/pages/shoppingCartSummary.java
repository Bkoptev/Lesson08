...
public class shoppingCartSummary extends AbstractPage{
    ...
    @FindBy(xpath = "//p[@class='alert alert-warning' and contains(text(),'Your shopping cart is empty.')]")
    ...
    public void comparePrice() {
        float beforePrice = Float.parseFloat(this.totalPrice.getText().replaceAll("^.*?(-?\\d+(\\.\\d+)?).*$", "$1"));
        addOneItem.click();
        testClass.waitTillElementIsVisible(quantity);
        float afterPrice = Float.parseFloat(this.totalPrice.getText().replaceAll("^.*?(-?\\d+(\\.\\d+)?).*$", "$1"));
        Assert.assertTrue("Total price was not changed or was changed not properly",beforePrice < afterPrice);
    }
